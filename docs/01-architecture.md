...
| **API server** (Node/Express) | Orchestrates pipeline: audio → ElevenLabs STT → Gemini → ElevenLabs TTS → session state → one payload back | Audio blob, session_id | JSON: transcript, Gemini decision object, TTS audio URL/base64 | Any upstream timeout (>6s) → cached fallback for current question index; fallback is signaled in-band (`degraded: true` + `degraded_components`, per `02-interfaces.md`); return `504 UPSTREAM_TIMEOUT` only if fallback cannot serve |
...
| **Gemini feedback engine** | Per turn: probe-further vs. finalize (≤2 follow-ups); on finalize: score 0–100, category, explanation + "what was great" + 2–3 tips, evidence-grounded | Question, role, prior turns, current transcript | Fixed-schema JSON (see `02-interfaces.md`) | Malformed JSON → 1 retry with responseSchema forced; still bad → rules-baseline fallback, tagged `evaluator: "fallback_rules"` |
...
| **Guardrail layer** (server-side pure functions) | Validates Gemini JSON: schema, evidence substring present in transcript, enum whitelists, feedback caps | Raw Gemini JSON, raw transcript | Validated/repaired JSON or fallback object | Any validation failure → replaced by rules-baseline fallback, never silently patched |
| **Eval harness** (standalone Node script, offline) | Runs labeled synthetic dialogues through rules-baseline, generic-prompt, Gemini pipeline; computes metrics | `eval/dataset.json` | `eval/results.json` + printed table | N/A — offline, no live-demo dependency |
...
flowchart TD
    ...
    subgraph External["Third-party APIs"]
        ...
        GEM["Gemini feedback engine<br/>(Google AI Studio REST)"]
    ...
    ORCH -->|question, role, prior answers, transcript| GEM
    GEM -->|structured JSON decision| GUARD
...
5. Server → Gemini with `{role, question, full_dialogue_so_far, turn_number}` → raw JSON decision.
6. Guardrail validates Gemini output (schema, enums, evidence substring). Valid → pass through. Invalid → rules-baseline fallback tagged `evaluator: "fallback_rules"`.
7. Route on Gemini's `next_action`:
...
- **Best Use of Gemini API (MLH):** steps 5–7 — the engine sits **between** transcript and UI, returning schema-enforced JSON (`responseSchema`, never free chat) the state machine executes verbatim. Raw JSON shown on-screen in the demo. Runs on the AI Studio free tier — $0 model spend, measured probe/finalize latency on Flash.
- **Best .Tech Domain Name (MLH):** the claimed .tech domain serves the static showcase build (hour 21–22); domain is on the Devpost submission.
...
| Hosting | **Local only** — server `localhost:8080`, client `localhost:5173` (Vite proxies `/api`) | No deploy flakiness, DNS, cold starts, or venue-Wi-Fi risk | `ngrok http 5173` at hour 22 if a public URL is required. Static showcase build (Vite, showcase mode, no backend) deployed to the .tech domain at hour 21–22 — the portfolio URL for Devpost and the .tech prize. |
...
| LLM | Gemini `POST https://generativelanguage.googleapis.com/v1beta/models/{GEMINI_MODEL}:generateContent`, `responseMimeType: "application/json"` + `responseSchema` | MLH Gemini prize req; plain fetch, no SDK dependency | Rules-baseline scorer, zero network |
...
**Env vars** (server unless noted): `ELEVENLABS_API_KEY`, `ELEVENLABS_VOICE_ID` (one pre-made neutral voice, picked once — no voice-hunting at the venue), `GEMINI_API_KEY`, `GEMINI_MODEL` (default `gemini-2.5-flash` — verify in AI Studio at build time), `PORT` (8080), `DB_PATH` (`./data/session.db`), `USE_BROWSER_STT_FALLBACK` (client build-time flag).
...
│   ├── decision-schema.json          # canonical feedback-engine output schema (content unchanged — model-agnostic)
...
│   ├── services/gemini.ts            # probeOrFinalize() -> raw JSON, 1 retry (Gemini REST + responseSchema)
...
│   └── prompts/feedbackPrompts.ts    # exact prompt strings, single source of truth
├── client/src/
