...
| 1:00–1:20 | "Here's what just happened under the hood." | Open `DecisionJsonPanel` | Raw Nemotron JSON: score, category, tips, evidence span highlighted against the transcript |
...
- **Why Nemotron, not just a bigger prompt?** "Nemotron returns a schema our state machine executes directly — routing, scoring, and follow-up choice are code decisions grounded in model output, not the model freelancing a conversation."
...
**Nemotron** — fixed-schema decision object, never free chat (§`02`/`03`); state machine executes `next_action`/`follow_up_type` verbatim, shown live via `DecisionJsonPanel`; full eval plan + documented miss (§`04`); five code-level guardrails (§`03`).
